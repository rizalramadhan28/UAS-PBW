...
/**
 * Cek apakah suatu nilai sudah dipakai pasien lain.
 * $exceptId dipakai saat edit agar record sendiri tidak dianggap duplikat.
 */
...
if (isset($_POST['simpan'])) {
    ...
    } elseif ($nik !== '' && pasien_field_exists($conn, 'nik', $nik)) {
        $error = 'NIK "' . htmlspecialchars($nik) . '" sudah terdaftar.';
    } elseif ($bpjs !== '' && pasien_field_exists($conn, 'no_bpjs', $bpjs)) {
        $error = 'No. BPJS "' . htmlspecialchars($bpjs) . '" sudah terdaftar.';
    } else {
